...
Map *createMap(int width, int height, int mines) {
    ...
    map->field = (bool**)malloc(sizeof(bool *) * height);
    map->indicator = (unsigned char**)malloc(sizeof(unsigned char *) * height);
    map->status = (unsigned char**)malloc(sizeof(unsigned char *) * height);
    for (int i = 0; i < height; ++i) {
        map->field[i] = (bool*)malloc(sizeof(bool) * width);
        map->indicator[i] = (unsigned char*)malloc(sizeof(unsigned char) * width);
        map->status[i] = (unsigned char*)malloc(sizeof(unsigned char) * width);
    }
    initMap(map);
    return map;
}
