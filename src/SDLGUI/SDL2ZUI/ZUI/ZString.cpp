...
namespace ZuiLib
{
	...
	{ 
		return SDL_strcasecmp(m_pstr, lpsz); 
	}
    ...
    int ZString::Format(const ZChar* pstrFormat, va_list Args)
    {
        ...
        nLen = SDL_vsnprintf(NULL, 0, pstrFormat, Args);
        totalLen = (nLen + 1)*sizeof(ZChar);
        szBuffer = (ZChar*)malloc(totalLen);
		memset(szBuffer,0,totalLen);
        nLen = SDL_vsnprintf(szBuffer, nLen + 1, pstrFormat, Args);
	...
	int ZString::SmallFormat(const ZChar* pstrFormat, ...)
	{
		...
		int iRet = SDL_vsnprintf(szBuffer, sizeof(szBuffer), sFormat, argList);
		va_end(argList);
		Assign(szBuffer);
		return iRet;
	}
