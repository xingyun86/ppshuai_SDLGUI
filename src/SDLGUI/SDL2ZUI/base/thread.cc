...
bool Thread::SleepMs(int milliseconds) {
	...
	if (ret != 0) {
		printf("Warning:nanosleep() returning early\n");
		return false;
	}
	return true;
#endif
...
bool Thread::Start() {
	...
	if (priority_ != PRIORITY_NORMAL) {
		if (priority_ == PRIORITY_IDLE) {
			...
			printf("Warning:PRIORITY_IDLE not supported\n");
		} else {
			...
			if (pthread_attr_getschedparam(&attr, &param) != 0) {
				...
				if (priority_ == PRIORITY_HIGH) {
					...
					if (priority_ == PRIORITY_ABOVE_NORMAL)
					{
	...
	if (0 != error_code) {
		printf("Error:Unable to create pthread, error %d\n", error_code);
		return false;
	}
	started_ = true;
#endif
